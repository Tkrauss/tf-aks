...
# It is not possible to have Terraform provision its remote state and then perform the actual provision
...
resource "azurerm_resource_group" "tf_aks_state" {
  ...
  tags     = "${merge(map("Name", "${var.projectname}-${var.location}-tfstate"), var.resource_tags)}"
}
